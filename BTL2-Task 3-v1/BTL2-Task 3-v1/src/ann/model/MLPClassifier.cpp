...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader){
        double_tensor X = batch.getData();// Lấy dữ liệu từ batch
        double_tensor Y = this->forward(X);// Đưa batch vào hàm forward

        // Cập nhật results bằng cách cộng dồn kết quả
        if(first_batch){
            results = Y;
            first_batch = false;
        }
        else{
            results = xt::concatenate(xt::xtuple(results, Y), 0);//results=
        }
        // Cập nhật số lượng mẫu và hiển thị thông tin
        nsamples += X.shape()[0];
        batch_idx++;
    }
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    for(auto batch: *pLoader){
        // Get data X and labels t from the batch
