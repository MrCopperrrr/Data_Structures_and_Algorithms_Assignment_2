...
xt::xarray<double> Softmax::backward(xt::xarray<double> deltaY) {
  ...
  double_tensor diag_Y = diag_stack(Y);
  double_tensor Y_outer = outer_stack(Y, Y);
  double_tensor Jacobian = diag_Y - Y_outer;
  double_tensor deltaZ = matmul_on_stack(Jacobian, deltaY);
